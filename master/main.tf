resource "azurerm_network_interface" "master_private_nic" {
    count                     = var.instance_count
 
    name                      = "master${count.index}-private-vnic"
    location                  = var.region
...
resource "azurerm_virtual_machine" "cacib_ocp_master_vm" {
    ...
    network_interface_ids = [element(azurerm_network_interface.master_private_nic.*.id, count.index)]
    vm_size               = var.vm_size

    primary_network_interface_id     = element(azurerm_network_interface.master_private_nic.*.id, count.index)
    ...
    provisioner "local-exec" {
      command = "echo ${azurerm_network_interface.master_private_nic[count.index].ip_configuration[0].private_ip_address} ${self.name} >> private-ips.txt"
    }
